...
(define-resource shop ()
  :class (s-prefix "veeakker:Shop")
  :properties `((:label :string ,(s-prefix "dct:title"))
                (:style :string ,(s-prefix "veeakker:customStyleSheet"))
                (:slug :string ,(s-prefix "veeakker:slug")))
  :has-one `((file :via ,(s-prefix "veeakker:logo")
                   :as "logo")
             (file :via ,(s-prefix "veeakker:topImage")
                   :as "top-image")
             (file :via ,(s-prefix "veeakker:placeholderImage")
                   :as "placeholder-image"))
  :has-many `((offering :via ,(s-prefix "veeakker:offerings")
                        :as "offerings")
              (delivery-place :via ,(s-prefix "veeakker:hasDeliveryPlace")
                              :as "delivery-places")
              (product-group :via ,(s-prefix "veeakker:disallowedProductGroup")
                             :as "disallowed-product-groups")
              (business-entity :via ,(s-prefix "veeakker:hasSupplier")
                               :as "suppliers"))
  :resource-base (s-url "http://veeakker.be/shops/")
  :on-path "shops")
...
(define-resource offering ()
  ...
  :has-many `((business-entity :via ,(s-prefix "gr:availableAtOrFrom")
              ...
              (shop :via ,(s-prefix "veeakker:offeredByShop")
                    :as "offered-by-shop"))
...
(define-resource order-line ()
  ...
  :has-one `((offering :via ,(s-prefix "veeakker:hasOffering")
                        :as "offering")
             (shop :via ,(s-prefix "veeakker:orderedFromShop")
                   :as "ordered-from-shop"))
